	...
	<div class="smn-frame">
		<header class="smn-header">
			<div class="smn-header-bar">
				<a class="smn-brand" href="{$gallery_catalog_url}">
			...
			<nav class="smn-breadcrumbs" aria-label="{if $lng eq 'eng'}Breadcrumbs{else}Хлебные крошки{/if}">
				...
				<span class="smn-breadcrumb-current">{if $lng eq 'eng'}Gallery{else}Галерея{/if}</span>
			</nav>
		</header>
		...
		<main class="smn-main">
			...
			<section class="smn-hero smn-hero--compact">
				<h1>{if $lng eq 'eng'}Gallery of Spectrum ezines{else}Галерея спектрумовских изданий{/if}</h1>
				<p class="smn-hero-sub">
					{if $lng eq 'eng'}
						{$gallery_total} screenshots · page {$gallery_page} of {$gallery_total_pages}
					{else}
						{$gallery_total} скриншотов · страница {$gallery_page} из {$gallery_total_pages}
					{/if}
				</p>
			</section>

			{assign var=gallery_pages_top value=true}
			{include file="gallery_new_pages.tpl"}

			{if $screens && $screens|@count gt 0}
			<ul class="smn-gallery">
			{foreach from=$screens item=shot}
				<li class="smn-gallery-item">
					<a class="smn-gallery-card" href="{$shot.gallery_issue_url}">
						<figure class="smn-gallery-figure">
							<img
								class="smn-gallery-img"
								src="{$shot.gallery_img_src}"
								alt="{$shot.gallery_alt}"
								width="256"
								height="192"
								loading="lazy"
								decoding="async"
							>
							<figcaption class="smn-gallery-caption">{$shot.gallery_label_plain}</figcaption>
						</figure>
					</a>
				</li>
			{/foreach}
			...
			{else}
			<p class="smn-empty-note">{if $lng eq 'eng'}No screenshots here yet.{else}Здесь пока нет скриншотов.{/if}</p>
			{/if}

			{assign var=gallery_pages_top value=false}
			{include file="gallery_new_pages.tpl"}
		</main>
