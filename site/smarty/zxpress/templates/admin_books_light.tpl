...
<form method='POST' enctype='multipart/form-data' action='admin_books_light.php?id={if $book.id}{$book.id}{else}0{/if}'>
...
<a href="admin_books_light.php?id=0" style="color: blue">Добавить новое издание +</a>
...
<a href="files/{$files[n].name}">{$files[n].file_name}</a> [{$files[n].file_size}Kb] <span style="font: 8pt verdana;">{$files[n].date}</span> &nbsp; 
