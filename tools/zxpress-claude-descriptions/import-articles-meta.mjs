#!/usr/bin/env node
/**
 * Local import: reads descriptions/*.txt, writes articles.meta_description_ru/en.
 *
 * Expected layout (run from zxpress-claude-descriptions/ on your Mac):
...
function normalizeMeta(value) {
	...
	if (t.length > META_MAX_LEN) {
		t = t.slice(0, META_MAX_LEN);
	}
	return t;
}
...
async function main() {
	...
	for (const path of files) {
		try {
			...
			if (st.isFile()) onlyFiles.push(path);
		} catch {
			// skip
		}
	}
	...
	try {
		if (!(await columnsExist(conn))) {
			console.error('[import] ERROR: run db/migration/articles_meta_description.sql on the server first');
			process.exit(1);
		}
